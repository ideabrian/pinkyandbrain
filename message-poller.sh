...
# Don't exit on error - we want to keep polling even if one iteration fails
set +e
...
while true; do
    log "INFO" "Polling for unread messages..."

    # Check for unread messages (not filtering by type since messages don't have .type field)
    UNREAD_TASKS=$(curl -s "$MESSAGE_BUS/inbox/unread" 2>/dev/null | \
        jq -r '.messages[]? | select(.to == "'"$AGENT_NAME"'" or .to == "any") | @json' 2>/dev/null)

    log "INFO" "Poll complete, checking results..."

    if [ ! -z "$UNREAD_TASKS" ]; then
        TASK_COUNT=$(echo "$UNREAD_TASKS" | grep -c '^{' || echo "0")
        if [ "$TASK_COUNT" -gt 0 ]; then
            log "INFO" "${BLUE}Found $TASK_COUNT unread message(s)${NC}"
    ...
    log "INFO" "Sleeping for ${POLL_INTERVAL}s..."
