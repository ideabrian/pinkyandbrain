...
while true; do
    ...
    if [ "$LOCAL_UNREAD" != "0" ] && [ "$LOCAL_UNREAD" != "" ]; then
        ...
        MESSAGE_SUBJECT=$(echo "$MESSAGE" | jq -r '.subject')
        MESSAGE_BODY=$(echo "$MESSAGE" | jq -r '.body')

        if [ "$MESSAGE_ID" != "null" ]; then
            # Skip system messages that create loops
            if [[ "$MESSAGE_SUBJECT" == "Session Ended" ]] || [[ "$MESSAGE_SUBJECT" == "Re: Auto-processed message" ]]; then
                log "${YELLOW}⏩ Skipping system message: $MESSAGE_SUBJECT${NC}"
